	          	...
	          	@if($data->count())
                    ...
                    @foreach($data as $r)
                        <tr>
                            ...
                            <td>
                            	<a href="{{route('portfolio.edit', ['id' => $r['id']])}}" class="btn btn-xs btn-warning"><i class="fa fa-edit"></i> Edit</a>
                                <form action="{{route('portfolio.destroy', ['id' => $r['id']])}}" method="POST" style="display: inline-block;">
                                    @method('DELETE')
                                    @csrf
